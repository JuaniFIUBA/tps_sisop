...
fisopfs_getattr(const char *path, struct stat *st)
{
	...
	printf("llego aca con la entrada %s\n", entrada->nombre);
	if (!entrada) {
		return -ENOENT;
	}
...
fisopfs_unlink(const char *path)
{
	...
	for (size_t i = 0; i < directorio->cantidad_entradas; i++) {
		...
		if (entrada && strcmp(entrada->nombre, nombre_archivo) == 0) {
...
{
	printf("[debug]fisop_symlink - to: %s, from: %s\n", to, from);

	// verifica el path
	if (strlen(to) > MAXIMO_PATH) {
		return -EINVAL;
	}
	// verifica si hay espacio en el file system para mas memoria
	if (fs.espacio_total_ocupado_fs + sizeof(entrada_enlace_simbolico_t) >
	    MAX_ESPACIO_FS) {
		return -ENOSPC;
	}

	char path_directorio_from[MAXIMO_PATH];
	obtener_directorio_path(from, path_directorio_from);

	char nombre_enlace_simbolico[MAXIMO_PATH];
	obtener_nombre_final_path(from, nombre_enlace_simbolico);

	entrada_directorio_t *directorio =
	        (entrada_directorio_t *) encontrar_entrada(path_directorio_from);
	...
	// crea el nuevo archivo
	entrada_enlace_simbolico_t *nuevo_enlace_simbolico =
	        malloc(sizeof(entrada_enlace_simbolico_t));

	if (!nuevo_enlace_simbolico) {
		return -ENOMEM;
	}

	int resultado = crear_enlace_simbolico(nuevo_enlace_simbolico,
	                                       nombre_enlace_simbolico,
	                                       to);

	if (resultado != 0) {
		free(nuevo_enlace_simbolico);
		return resultado;
	}

	// lo coloca en el directorio
	resultado = colocar_entrada_dentro_de_directorio(
	        directorio, (entrada_t *) nuevo_enlace_simbolico);
	...
	// actualiza el espacio ocupado por el file system
	fs.espacio_total_ocupado_fs += sizeof(entrada_enlace_simbolico_t);
	return 0;
}
...
{
	printf("[debug]fisop_readlink -path:%s, buf:%s, size:%li\n", path, buf, size);
	...
	entrada_enlace_simbolico_t *enl_sim =
	        (entrada_enlace_simbolico_t *) encontrar_entrada(path);

	if (!enl_sim || enl_sim->tipo != ENLACE_SIMBOLICO) {
		return -ENOENT;
	}

	strncpy(buf, enl_sim->enlace, size);
	// actuliza el enlace
	enl_sim->st.st_atime = time(NULL);
