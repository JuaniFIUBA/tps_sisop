...
crear_directorio_desde_archivo(FILE *archivo)
{
	...
	for (int j = 0; j < directorio->cantidad_entradas_ocupadas; j++) {
		...
		case ENLACE_SIMBOLICO:
			...
			// lee el enlace simbolico
			resultado = fread(enl_sim,
			                  sizeof(entrada_enlace_simbolico_t),
			                  1,
			                  archivo);
			...
			directorio->entradas[j] = (entrada_t *) enl_sim;
			break;
		default:
...
escribir_directorio_en_archivo(entrada_directorio_t *directorio, FILE *archivo)
{
	...
	for (int j = 0; j < directorio->cantidad_entradas; j++) {
		...
		case ENLACE_SIMBOLICO:
			...
			resultado = fwrite((entrada_enlace_simbolico_t *) entrada,
			                   sizeof(entrada_enlace_simbolico_t),
			                   1,
			                   archivo);
